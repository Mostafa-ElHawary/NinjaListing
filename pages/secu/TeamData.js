...
export default TeamData;
{
  ...
  //           <h3 className='text-7 font-semibold'>{project.name}</h3>
  //           {project.features.map((feature) => (
  //             <div  key={feature.name} >
  //               <h4 className='font-bold' >{feature.name}</h4>
  //               {feature.topics.map((topic) => (
  //                 <div   key={topic.name}>
  //                   <h5 className='font-bold text-blue-700' >{topic.name}</h5>
  //                   <ul>
  //                     {topic.steps.map((step) => (
  //                       <li key={step.title}>
  //                         <h6 className='font-black'>{step.title}</h6>
  //                         <p className='text-xl font-semibold text-gray-500'>{step.description}</p>
  //                       </li>
  //                     ))}
  //                   </ul>
  //                 </div>
