...
export default function Porfoliocard({ params }) {
  ...
  return (
    ...
    <div  className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-2 lg:grid-cols-3 gap-8 max-w-screen-xl mx-auto">
    {projectsData.map((project) => (
      <div key={project.id} className="grid grid-row-1 gap-8">
        {project.teaminfo.map((card) => (
          <div
            key={card.id}
            className="bg-white rounded-lg p-4 mx-4 sm:mx-auto sm:max-w-md border border-gray-300 shadow-md"
          >
            <div className="mb-4">
              <img
                src={card.url}
            ...
            <div className="flex items-center justify-center mb-2">
              ...
              <div className="text-gray-600 ml-1 text-sm"> {card.phoneNumber} </div>
            </div>
